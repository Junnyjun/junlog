...
`system`은 테이블에 데이터가 없거나 한 개만 있는 경우로, 성능상 최상의 `type`이라고 할 수 있습니다.

#### CONST

`const`는 조회되는 데이터가 단 1건일 때 출력되는 유형으로, 성능상 매우 유리한 방식입니다. 고유 인덱스나 기본키를 사용하여 단 1건의 데이터에만 접근하면 되므로, 속도나 리소스 사용 측면에서 지향해야 할 타입입니다.
